...
public abstract class MicroService implements Runnable {
    /* we got a premition from Matan to add fields to  this class */

    protected MessageBusImpl messageBusInstance;
    protected Diary diaryInstance;

    protected String name;
    protected boolean terminated;

    protected HashMap<Class<? extends Message>, Callback> messageCallback;
    protected HashMap<Message, Future> messagesToFollow;

    protected static CountDownLatch countDownRegister = new CountDownLatch(4);
    ...
    public MicroService(String name) {
        this.name = name;
        this.messageBusInstance = MessageBusImpl.getInstance();
        this.diaryInstance = Diary.getInstance();
        this.terminated = false; // flag for the message loop
        this.messageCallback = new HashMap<>();
        this.messagesToFollow = new HashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        this.messageBusInstance.subscribeEvent(type, this);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        this.messageCallback.put(type, callback);
        this.messageBusInstance.subscribeBroadcast(type, this);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return this.messageBusInstance.sendEvent(e);
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
        this.messageBusInstance.sendBroadcast(b);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        this.messageBusInstance.complete(e, result);
    }
    ...
    public final void run() {
        ...
        initialize();
        countDownRegister.countDown();   /*"[Count = " + sync.getCount() + "]"*/
        while (!terminated) {
            try {
                messageLoop();
    ...
    private void messageLoop() throws InterruptedException {
        /* the MS will wait for a message */
        Message msgToDo = messageBusInstance.awaitMessage(this);
        (messageCallback.get(msgToDo.getClass())).call(msgToDo);
    }
}
